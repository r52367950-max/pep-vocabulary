...
export default function StudyAiCoach({ enabled, entry, question, answer }: { enabled: boolean; entry: LexiconIndexEntry; question: Question; answer: string }) {
  ...
  return <section className="study-ai-coach">
    <header><div><Bot size={16}/><strong>AI 深度精讲</strong><small>单词回答上限 20,000 tokens · 不参与评分或 FSRS 排程</small></div><span><button onClick={() => run("explain")} disabled={!enabled || loading !== null}>{loading === "explain" ? "深度生成中…" : question.type === "sentence-output" ? "检查这句话" : "精讲本题"}</button><button onClick={() => run("practice")} disabled={!enabled || loading !== null}>{loading === "practice" ? "生成中…" : "出 2 道加练"}</button></span></header>
    {!enabled && <p className="study-ai-note">AI 未启用；当前题仍可完整学习和评分。</p>}
    {error && <p className="ai-insight-error"><CircleAlert size={14}/>{error}</p>}
    {result?.kind === "explain" && <div className="study-ai-result deep-explain-result"><p><Sparkles size={14}/>{result.summary}</p><h4>核心义与用法</h4>{result.meaning.map((line, index) => <span key={`meaning-${index}`}>{line}</span>)}<h4>语法</h4>{result.grammar.map((line, index) => <span key={`grammar-${index}`}>{line}</span>)}<h4>搭配与辨析</h4>{result.collocations.map((line, index) => <span key={`collocation-${index}`}>{line}</span>)}<h4>分层例句</h4>{result.examples.map((example, index) => <blockquote key={`example-${index}`}>{example.sentence}<small>{example.translation} · AI 新写例句</small></blockquote>)}</div>}
