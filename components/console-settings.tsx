        ...
        {section === "ai" && <>
          ...
          <section className="settings-card api-form">
            ...
            <div className="form-grid">
              ...
              <label><span>请求超时</span><div className="suffix-input"><input type="number" min="10" max="60" value={aiConfig.timeoutSeconds} onChange={(event) => setAIConfig((current) => ({ ...current, timeoutSeconds: Number(event.target.value) }))}/><span>秒</span></div><small>一般请求建议 20–30 秒；深度任务会按输出预算自动延长等待。</small></label>
            </div>
          ...
          <section className="settings-card compact-card"><header><Eye size={19}/><div><h2>发送范围与 token 预算</h2><p>搜索先在本机召回最多 12 个词；分析与规划只发送匿名聚合；背诵只发送正式词条 ID。搜索 5k–8k、分析 7k–12k、计划 9k–16k，按请求规模动态分配；单词精讲上限 20k。固定前缀与规范 JSON 保持缓存稳定。</p></div></header></section>
        </>}
