  ...
  return <div className="settings-console">
    ...
    <div className="settings-layout"><nav className="settings-nav" aria-label="设置分类">{sections.map((item) => <button key={item.id} className={section === item.id ? "active" : ""} onClick={() => { setSection(item.id); setMessage(null); }}><item.icon size={17}/><span>{item.label}</span><ChevronRight size={14}/></button>)}</nav>
      <main className="settings-content">
        {section === "general" && <><section className="settings-card"><header><Palette size={20}/><div><h2>外观</h2><p>只改变本机显示，不影响学习记录。</p></div></header><div className="settings-row"><div><strong>配色方案</strong><small>可跟随当前设备的浅色或深色设置</small></div><div className="segmented-control" role="group" aria-label="配色方案">{(["system", "light", "dark"] as const).map((theme) => <button key={theme} className={settings.theme === theme ? "active" : ""} onClick={() => onUpdate({ theme })}>{theme === "system" ? "系统" : theme === "light" ? "浅色" : "深色"}</button>)}</div></div></section><section className="settings-card"><header><Sparkles size={20}/><div><h2>增强功能</h2><p>AI 只承担讲解、例句与辨析，FSRS 排程始终在本地完成。</p></div></header><label className="settings-row toggle-row"><div><strong>启用 AI 增强</strong><small>{aiConfig.hasApiKey ? "接口已就绪；关闭后不会发起模型请求" : "需要先在“AI 接口”中保存密钥"}</small></div><input type="checkbox" checked={settings.aiEnabled} onChange={(event) => onUpdate({ aiEnabled: event.target.checked })}/><span className="switch"/></label></section></>}
        ...
        {section === "ai" && <>
          ...
          <section className="settings-card compact-card"><header><Eye size={19}/><div><h2>发送范围</h2><p>四类助手接口只发送当前词条 ID、必要作答文本、有限掌握标签，以及服务端重新读取的正式词库证据。</p></div></header></section>
        </>}
        ...
        {section === "about" && <section className="settings-card about-card"><header><Laptop size={20}/><div><h2>词迹</h2><p>人教版初高中英语词汇学习 · 本地优先</p></div></header><dl><div><dt>应用版本</dt><dd>1.3.1</dd></div><div><dt>用户数据 schema</dt><dd>1.1.0</dd></div><div><dt>词库发布状态</dt><dd>带已声明缺口的发布候选</dd></div><div><dt>AI 密钥边界</dt><dd>服务端加密，不随备份导出</dd></div></dl></section>}
      </main>
    </div>
  </div>;
}
