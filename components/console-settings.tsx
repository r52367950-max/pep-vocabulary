...
const sections = [
  ...
  { id: "learning" as const, label: "学习", icon: BookOpenCheck },
  { id: "ai" as const, label: "AI 接口", icon: Bot },
  { id: "privacy" as const, label: "数据与安全", icon: ShieldCheck },
  { id: "about" as const, label: "关于", icon: CircleAlert },
];
const apiHeaders = () => ({
  "content-type": "application/json",
  "x-vocab-action": "settings",
});
  ...
  useEffect(() => {
    ...
    fetch("/api/ai/config", { cache: "no-store" })
      .then(async (response) => {
        ...
        if (!response.ok) throw new Error(result.error || "无法读取 API 配置");
        if (active) setAIConfig(result);
      })
      .catch(
        (error) =>
          active &&
          setMessage({
            kind: "error",
            text: error instanceof Error ? error.message : "无法读取 API 配置",
          }),
      )
              ...
              <section className="settings-card">
                ...
                <label className="settings-slider">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      onUpdate({ dailyMinutes: Number(event.target.value) })
                    }
                  />
                  <small>积压较多时，系统会先减少新词。</small>
                </label>
                <label className="settings-slider">
                  <div>
                    <strong>目标保持率</strong>
                    <span>{Math.round(settings.desiredRetention * 100)}%</span>
                  </div>
                  <input
                    type="range"
                    min="0.8"
                    ...
                    onChange={(event) =>
                      onUpdate({ desiredRetention: Number(event.target.value) })
                    }
                  />
                  <small>保持率越高，每日复习量通常越大。</small>
                </label>
              </section>
              <section className="settings-card">
                <header>
                ...
                <label className="settings-row">
                  ...
                  >
                    <option value="normal">普通学习</option>
                    <option value="unit">单元同步</option>
                    <option value="review-only">只复习</option>
                    <option value="exam">考前强化</option>
                    <option value="browse">自由浏览</option>
                  </select>
                </label>
              </section>
            </>
            ...
            <>
              <section className="settings-card api-overview">
                <header>
                  ...
                  <div>
                    <h2>模型 API</h2>
                    <p>
                      支持 DeepSeek 与 OpenAI-compatible Chat
                      Completions；请求只从站点服务端发出。
                    </p>
                  </div>
                  <span
                    className={
                      aiConfig.hasApiKey ? "api-badge ready" : "api-badge"
                    }
                  >
                    {loadingConfig
                      ? "读取中"
              ...
              <section className="settings-card api-form">
                ...
                <div className="form-grid">
                  <label>
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        chooseProvider(event.target.value as AIProvider)
                      }
                    >
                ...
                <p className="connection-test-note">
                  测试先进行一次不携带密钥、不会计费的 HTTPS
                  探测，再发送两次极小的真实请求检查基础生成与 JSON
                  输出；会产生少量服务商用量。
                </p>
                {message && (
                  <p
          ...
          {section === "privacy" && (
            <>
              <section className="settings-card">
                ...
                <button className="settings-link-row" onClick={onOpenData}>
                  <div>
