...
export default function ReadingImport({ onClose, onSaved }: { onClose: () => void; onSaved: (id: string) => void }) {
  ...
  const [body, setBody] = useState(""); const deferredBody = useDeferredValue(body); const words = useMemo(() => countReadingWords(deferredBody), [deferredBody]); const [category, setCategory] = useState<ReadingCategory>("essay"); const [difficulty, setDifficulty] = useState<Difficulty>("B1");
  ...
  return <dialog ref={dialogRef} className="import-dialog" aria-labelledby="import-title" data-closing={closing} onCancel={onCancel} onPointerDown={onPointerDown} onClick={onClick}>
    <div className="import-frame"><header className="settings-dialog-header"><h2 id="import-title">导入文章</h2><button className="icon-button" aria-label="关闭文章导入" onClick={requestClose}><X size={20} aria-hidden="true" /></button></header>
      <div className="import-content"><div className="import-methods"><button disabled={busy} onClick={() => input.current?.click()}><FileUp size={22} aria-hidden="true" /><strong>选择文件</strong><span>PDF、Word 或文本</span></button><button disabled={busy} onClick={() => camera.current?.click()}><Camera size={22} aria-hidden="true" /><strong>扫描英文</strong><span>拍照或选择图片</span></button></div>
        ...
        <label className="import-body-label"><span>正文<span>{words.toLocaleString("zh-CN")} 词{words < 40 ? " · 至少需要 40 词" : ""}</span></span><textarea aria-label="导入文章正文" rows={8} value={body} maxLength={60000} disabled={busy} onChange={e => setBody(e.target.value)} placeholder="在这里粘贴英文，或先选择文件…" /></label>
      ...
      </div><footer className="import-footer"><span>文章保存在本机</span>{busy ? <button className="secondary" onClick={cancel}>取消处理</button> : <button className="primary" onClick={() => void save()} disabled={!title.trim() || words < 40}>保存并阅读</button>}</footer>
    </div>
  </dialog>;
}
