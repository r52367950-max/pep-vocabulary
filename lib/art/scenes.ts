...
// ---------------------------------------------------------------------------
// 几何构成: a quiet grid of circles, arcs and stripes with one large form.
// ---------------------------------------------------------------------------
const GEOMETRY = [
  { bg: "#ecebe6", ink: "#1f2430", colors: ["#2f4b7c", "#d9a33f", "#c8553d", "#8aa899"] },
  { bg: "#1f2a3d", ink: "#f1ede3", colors: ["#e3b448", "#d7694e", "#8fb3c7", "#f1ede3"] },
  { bg: "#dfe5df", ink: "#1f2a26", colors: ["#2d5a4e", "#e0a95a", "#b75d48", "#f4f1ea"] },
  { bg: "#efe7e1", ink: "#2a2224", colors: ["#7d4e6d", "#3f6f8f", "#e2a55c", "#c9b6a8"] },
];
function motif(ctx: Ctx, x: number, y: number, s: number, color: string, ink: string, rng: Rng) {
  const p = s * 0.12, cx = x + s / 2, cy = y + s / 2, R = s / 2 - p, kind = rng.int(0, 7), rot = (rng.int(0, 3) * Math.PI) / 2;
  ctx.fillStyle = color;
  ctx.strokeStyle = ink;
  ctx.lineWidth = 1.2;
  if (kind === 0) { ctx.beginPath(); ctx.arc(cx, cy, R, 0, TAU); ctx.fill(); }
  else if (kind === 1) { ctx.beginPath(); ctx.arc(cx, cy, R, rot, rot + Math.PI); ctx.closePath(); ctx.fill(); }
  else if (kind === 2) {
    const corners = [[x + p, y + p], [x + s - p, y + p], [x + s - p, y + s - p], [x + p, y + s - p]], [kx, ky] = corners[rng.int(0, 3)];
    ctx.save();
    ctx.beginPath(); ctx.rect(x + p, y + p, s - 2 * p, s - 2 * p); ctx.clip();
    ctx.beginPath(); ctx.arc(kx, ky, s - p * 2, 0, TAU); ctx.fill();
    ctx.restore();
  } else if (kind === 3) { for (let i = 1; i <= 4; i++) { ctx.beginPath(); ctx.arc(cx, cy, R * (i / 4), 0, TAU); ctx.stroke(); } }
  else if (kind === 4) {
    ctx.save();
    ctx.beginPath(); ctx.arc(cx, cy, R, 0, TAU); ctx.clip();
    for (let j = -6; j <= 6; j++) { ctx.beginPath(); ctx.moveTo(cx + (j * R) / 5, cy - R); ctx.lineTo(cx + (j * R) / 5, cy + R); ctx.stroke(); }
    ctx.restore();
  } else if (kind === 5) { for (let gx = 0; gx < 4; gx++) for (let gy = 0; gy < 4; gy++) dot(ctx, x + p + ((gx + 0.5) * (s - 2 * p)) / 4, y + p + ((gy + 0.5) * (s - 2 * p)) / 4, s * 0.035, ink); }
  else if (kind === 6) { ctx.beginPath(); ctx.moveTo(x + p, y + s - p); ctx.lineTo(x + s - p, y + s - p); ctx.lineTo(rng.chance(0.5) ? x + p : x + s - p, y + p); ctx.closePath(); ctx.fill(); }
  else ctx.fillRect(x + p, cy - R * 0.18, s - 2 * p, R * 0.36);
}
export const geometry: Scene = (ctx, w, h, rng) => {
  const pal = rng.pick(GEOMETRY);
  paper(ctx, w, h, pal.bg, rng, { blots: 6 });
  const cols = rng.int(4, 6), cell = w / cols, rows = Math.ceil(h / cell), offY = (h - rows * cell) / 2;
  const colors = rng.shuffle(pal.colors).slice(0, 3);
  const hc = rng.int(0, cols - 2), hr = rng.int(0, Math.max(0, rows - 2));
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      if (c - hc >= 0 && c - hc < 2 && r - hr >= 0 && r - hr < 2) continue;
      if (rng.chance(0.3)) continue;
      motif(ctx, c * cell, offY + r * cell, cell, rng.pick(colors), pal.ink, rng);
    }
  }
  const bx = hc * cell, by = offY + hr * cell, bs = cell * 2, big = rng.int(0, 2);
  ctx.fillStyle = colors[0];
  if (big === 0) {
    ctx.beginPath(); ctx.arc(bx + bs / 2, by + bs / 2, bs * 0.46, 0, TAU); ctx.fill();
    ctx.fillStyle = colors[1];
    ctx.beginPath(); ctx.arc(bx + bs / 2, by + bs / 2, bs * 0.46, Math.PI * 0.5, Math.PI * 1.5); ctx.fill();
  } else if (big === 1) {
    ctx.beginPath(); ctx.moveTo(bx, by + bs); ctx.arc(bx, by + bs, bs, -Math.PI / 2, 0); ctx.closePath(); ctx.fill();
    ctx.strokeStyle = pal.ink;
    ctx.lineWidth = 1.2;
    for (let k = 1; k < 5; k++) { ctx.beginPath(); ctx.arc(bx, by + bs, bs * (k / 5), -Math.PI / 2, 0); ctx.stroke(); }
  } else {
    ctx.beginPath(); ctx.arc(bx + bs / 2, by + bs / 2, bs * 0.46, 0, TAU); ctx.fill();
    ctx.fillStyle = pal.bg;
    ctx.beginPath(); ctx.arc(bx + bs / 2 + bs * 0.12, by + bs / 2 - bs * 0.1, bs * 0.3, 0, TAU); ctx.fill();
  }
  const ly = offY + rng.int(1, Math.max(1, rows - 1)) * cell;
  stroke(ctx, [{ x: 0, y: ly }, { x: w, y: ly }], pal.ink, 1.2);
  dot(ctx, rng.range(0.1, 0.9) * w, ly, 4, pal.ink);
};
